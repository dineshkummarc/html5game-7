...
SelectDialog = function() {
	...
	this.process = function(key) {
		...
		// リターンキーが押されたら選択実行
		if (this._process_stage == SelectDialog.PROCESS_STAGE_NONE && key.getTrigger() != 0) {
			this._process_stage = SelectDialog.PROCESS_STAGE_DONE;
		}

		// 選択済みでリターンキーが解放されたら終了状態としてtrueを返す
		if (this._process_stage == SelectDialog.PROCESS_STAGE_DONE && key.getTrigger() == 0) {

			this._process_stage = SelectDialog.PROCESS_STAGE_EXIT;
		...
		// ウエイト中でなければ、上下キー入力処理
		if (this._process_stage == SelectDialog.PROCESS_STAGE_NONE && this._key_wait == 0) {
...
SelectDialog.PROCESS_STAGE_DONE = 1;
